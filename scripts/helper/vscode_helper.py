...
def update_vscode_settings(src, dst):
    ...
    for file in vscode_files:
        print(f'\t- {file}', end='')
        sh.copy(src=os.path.join(src, file), 
...
def update_settings():
    ...
    update_vscode_settings(src=src, 
                               dst=dst)
...
def main():
    assert len(sys.argv) == 0, 'No argument provided, please use either "update" or "install"'
    assert len(sys.argv) > 1, 'Too many arguments provided, please use either "update" or "install"'
