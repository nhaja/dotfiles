				...
				integrations = {
					...
					markdown = {
						...
						resolve_image_path = function(document_path, image_path, fallback)
							...
							local vault_dir = "/Users/nhaja/Obsidian/Documents/Notes"
							local image_dir = ""
							if working_dir:find(vault_dir) then
								image_dir = working_dir .. "/" .. image_path
							end
							if vim.fn.filereadable(image_dir) == 1 then
								return image_dir
							else
								image_dir = find_file(vault_dir, image_path)
								if vim.fn.filereadable(image_dir) == 1 then
									return image_dir
								end
							end
